...
// Stage 2: interpretation. Requires a capture in "ready" (or "interpreted") state.
// provider: "openai" (default) or "anthropic" — the bake-off switch.
app.post("/interpret", (req, res) => {
  ...
  const {
    ...
    model,                     // optional override; sensible default per provider
    callback_url,
    max_frames = 40,
  } = req.body || {};
  ...
  res.status(202).json({ accepted: true, session_id, provider });
